...
// BẢO VỆ TÀI KHOẢN ADMIN: Quản Lý Vùng (không phải Admin gốc) không được đụng
// vào BẤT KỲ tài khoản Admin nào — chỉ Admin mới quản lý được Admin (kể cả
// Admin khác, để tránh tình trạng tạo xong 1 Admin mới là không ai còn thu hồi
// quyền được nữa nếu cần — VD lộ PIN, nhân sự nghỉ việc).
...
function blockedFromAdminAccount(req, targetEmployee) {
  return !!(targetEmployee && targetEmployee.isAdmin && !req.employee.isAdmin);
}
