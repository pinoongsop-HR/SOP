...
router.put('/:id', requireAuth, requireAdmin, (req, res) => {
  ...
  // BẢO VỆ ADMIN: Quản Lý Vùng không được đụng vào bất kỳ tài khoản Admin nào.
  // Admin thì được sửa Admin khác (kể cả chính mình) — tránh tình trạng tạo
  // xong 1 Admin mới là vĩnh viễn không ai thu hồi quyền được nữa.
  if (blockedFromAdminAccount(req, emp)) {
    return res.status(403).json({ error: 'Chỉ Admin mới được sửa tài khoản Admin (Quản Lý Vùng không có quyền này).' });
  }
  ...
  // Không cho thu hồi quyền Admin của người CUỐI CÙNG còn giữ quyền này —
  // tránh cả hệ thống không còn ai có quyền tối thượng.
  if (isAdmin === false && emp.isAdmin) {
    const otherAdmins = db.employees.filter((e) => e.isAdmin && e.id !== emp.id).length;
    if (otherAdmins === 0) return res.status(400).json({ error: 'Không thể thu hồi quyền Admin của người cuối cùng — hãy cấp Admin cho người khác trước.' });
  }
...
router.delete('/:id', requireAuth, requireAdmin, (req, res) => {
  ...
  // Admin được xoá Admin khác (kể cả Admin do mình tạo ra) — nhưng KHÔNG được
  // xoá Admin CUỐI CÙNG của hệ thống (tránh mất quyền truy cập vĩnh viễn).
  // Quản Lý Vùng thì tuyệt đối không xoá được bất kỳ Admin nào.
  if (target.isAdmin) {
    if (!req.employee.isAdmin) return res.status(403).json({ error: 'Chỉ Admin mới được xoá tài khoản Admin (Quản Lý Vùng không có quyền này).' });
    const otherAdmins = db.employees.filter((e) => e.isAdmin && e.id !== target.id).length;
    if (otherAdmins === 0) return res.status(400).json({ error: 'Không thể xoá Admin cuối cùng của hệ thống — hãy cấp Admin cho người khác trước.' });
  }
