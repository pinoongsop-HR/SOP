...
router.post('/:employeeId', requireAuth, (req, res) => {
  ...
  if (!ids.includes(req.params.employeeId) || req.params.employeeId === req.employee.id) {
    if (!isPrivileged(req.employee)) return res.status(403).json({ error: 'Bạn không có quyền đánh giá nhân sự này' });
  }
  ...
  if (!emp.position) return res.status(400).json({ error: 'Tài khoản quản trị không tham gia KPI, không cần đánh giá định kỳ' });
