...
async function renderDetail(employeeId) {
  ...
  if (!emp.position) {
    const editLocked = emp.isAdmin && emp.id !== ME.id;
    panel.innerHTML = `
      ...
      <div class="emp-detail-body">
        ...
        ${emp.isAdmin ? `<div class="review-hint">${editLocked ? 'Không ai sửa được tài khoản Admin này, kể cả Admin khác — chỉ chính chủ tự sửa.' : 'Bạn có thể tự sửa thông tin / đổi PIN của mình ở nút "Đổi mã PIN" trên sidebar, hoặc qua Quản lý nhân sự.'}</div>` : ''}
      </div>
    `;
...
async function renderStaffView() {
  ...
  body.innerHTML = list.map((e) => {
    // KHOÁ ADMIN TUYỆT ĐỐI: không ai sửa được 1 tài khoản Admin ngoài chính họ;
    // không ai xoá được tài khoản Admin (kể cả chính họ) qua giao diện này.
    const editLocked = e.isAdmin && e.id !== ME.id;
    const deleteLocked = !!e.isAdmin;
    ...
    <tr>
      ...
      <td style="white-space:nowrap;">
        ...
        <button class="btn btn-sm" data-edit="${e.id}" ${editLocked ? 'disabled title="Không ai sửa được tài khoản Admin, chỉ chính chủ tự sửa"' : ''}>Sửa</button>
        <button class="btn btn-danger btn-sm" data-del="${e.id}" ${deleteLocked ? 'disabled title="Không ai xoá được tài khoản Admin"' : ''}>Xoá</button>
      </td>
    </tr>
  `;
  }).join('');
